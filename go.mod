module github.com/rogeliodh/mosquitto-exporter

go 1.14

require (
	github.com/beorn7/perks v0.0.0-20160804104726-4c0e84591b9a
	github.com/codegangsta/cli v1.20.1-0.20170813145949-f017f86fccc5
	github.com/eclipse/paho.mqtt.golang v1.0.1-0.20161224122424-d4f545eb108a
	github.com/golang/protobuf v0.0.0-20161117033126-8ee79997227b
	github.com/matttproud/golang_protobuf_extensions v1.0.1
	github.com/prometheus/client_golang v0.8.1-0.20170215133833-9c2f7254672d
	github.com/prometheus/client_model v0.0.0-20150212101744-fa8ad6fec335
	github.com/prometheus/common v0.0.0-20170108231212-dd2f054febf4
	github.com/prometheus/procfs v0.0.0-20170214153519-a59ae619c824
	golang.org/x/net v0.0.0-20170215215908-b4690f45fa1c
)
